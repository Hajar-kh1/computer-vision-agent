    ...
    const check = async () => {
      try {
        ...
        setLabel(
          ok
            ? 'Backend online'
            : `Backend: db=${h.database}, model=${h.model}`
        )
      } catch {
        if (!alive) return
        ...
        <div className="card">
          ...
          <ul>
            {result.top_predictions.map((t) => (
              <li key={t.class_name}>
                {t.class_name}: {pct(t.probability)}
              </li>
            ))}
          </ul>

          <p className="muted">
            Latency {result.inference_ms} ms · model v
  ...
  return (
    <section className="view">
      ...
      {rows && rows.items.length === 0 && (
        <p>
          No predictions yet — classify an image first.
        </p>
      )}
      ...
      {stats && total > 0 && (
        <>
          ...
          <div className="cards">
            ...
            <div className="stat">
              <span className="stat-num">
                {stats.avg_inference_ms != null
                  ? stats.avg_inference_ms + ' ms'
                  : '—'}
              </span>
              {' '}avg latency
            </div>
